...
std::string clDeviceTypeToString(cl_device_type type)
{
	...
	case CL_DEVICE_TYPE_CUSTOM:
		return "CUSTOM";
	default:
		throw std::invalid_argument("incorrect cl_device_type: " + to_string(type));
	}
}
...
int main()
{
	...
	for(cl_uint platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(cl_uint deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "            Device type: " << clDeviceTypeToString(deviceType) << std::endl;
			...
			std::cout << "            Device memory size: " << deviceMemSize / 1048576 << " MiB" << std::endl;
			...
			cl_bool isDeviceAvailable = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_AVAILABLE, sizeof(cl_bool), &isDeviceAvailable, nullptr));

			std::cout << "            Device available: " << (isDeviceAvailable == CL_TRUE ? "Yes" : "No") << std::endl;
		}
	}
